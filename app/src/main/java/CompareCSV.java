...
public class CompareCSV {
    ...
    public CompareCSV(String file_name1, String file_name2) throws IOException {
    ...
    public String compareCSV() throws IOException {
        ...
        Scanner user_input = new Scanner(System.in);  // Create a Scanner object to take user input
        for (Object key : unique_comb_to_choose.keySet()) {
            Object value = unique_comb_to_choose.get(key);
            System.out.printf("%d. %s%n",key,value);
        }
        System.out.println("Please give the unique combination in terms of column index(e.g. 0,1,2):");

        String userInput = user_input.nextLine();  // Read user input
        String[] uniq_comb = userInput.split(",");
        try {
            for (String col: uniq_comb) {
                uniqComb.add(Integer.parseInt(col));
            }
        }catch (Exception e){
